...
describe("Context Routes", () => {
    ...
    test("should set and get context with key", async () => {
        ...
        // Verify set response
        expect(setRes._status).toBe(200);
        expect(setRes._json).toHaveProperty("message", "Context successfully set");
        // Get context
        ...
        expect(getRes._json).toHaveProperty("context");
        expect(getRes._json.context.key).toBe(testKey);
        expect(getRes._json.context.message).toBe(testMessage);
        expect(getRes._json.context.metadata).toEqual({ type: "context" });
    });
    test("should handle validation errors for missing key", async () => {
        ...
        expect(res._json).toHaveProperty("error", "Key field is required");
    });
    test("should handle non-existent context key", async () => {
        ...
        expect(res._status).toBe(400);
        expect(res._json).toHaveProperty("error", "Context not found");
    });
