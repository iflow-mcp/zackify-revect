...
const setupMocks = (storedModel, storedSize) => {
    ...
    mock.module("../src/database/metadata", () => ({
        ...
        getMetadataValue: mock((key) => {
            if (key === "AI_EMBEDDING_MODEL")
                return storedModel;
            if (key === "AI_EMBEDDING_SIZE")
                return storedSize;
            return null;
        }),
...
describe("Embedding Model Change Detection", () => {
    ...
    test("should detect when embedding model changes", async () => {
        const { mockReembedAllDocuments } = setupMocks("text-embedding-ada-002", "1536");
    ...
    test("should detect when embedding size changes", async () => {
        const { mockReembedAllDocuments } = setupMocks("text-embedding-3-small", "1536");
        // Set new environment variables
        process.env.AI_EMBEDDING_MODEL = "text-embedding-3-small";
