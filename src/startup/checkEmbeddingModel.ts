  ...
  try {
    ...
    if (
      ...
      if (storedModel !== currentModel) {
        console.log(`- Model changed from ${storedModel || "not set"} to ${currentModel}`);
      }
      
      if (currentSize && storedSize !== currentSize) {
        console.log(`- Size changed from ${storedSize || "not set"} to ${currentSize}`);
      }
      
      // Re-embed all documents and chunks
      await reembedAllDocuments();
    } else {
      console.log(`Using embedding model: ${currentModel}${currentSize ? `, size: ${currentSize}` : ''}`);
    }
  } catch (error) {
